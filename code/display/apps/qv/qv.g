# qv.g: Quick interface to image display
#
...
#
#   $Id: qv.g,v 19.1 2005/06/15 18:09:56 cvsmgr Exp $

pragma include once;

include 'ddlws.g';

qv := function(imfile, ws=ddlws) {
...
#
# Write a jpg version of the image
#
  public.writejpg := function(filename='') {
    wider private, public;
    if(filename=='') {
      filename := spaste(private.imfile.name(), '.jpg');
    }
#    xpmname := spaste(filename, '.xpm');
    psname := spaste(filename, '.ps');
    note('Printing jpg image to ', filename);
#    private.dp.canvasprintmanager().writexpm(xpmname);
    private.dp.canvasprintmanager().writeps(psname);
#    return shell(spaste('xpmtoppm ', xpmname, ' | ppmtojpeg > ', filename));
    return shell(spaste('convert ', psname, ' ', filename));
  }
#

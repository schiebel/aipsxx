...
//#
//# $Id: ColormapDefinition.cc,v 19.5 2005/06/15 17:56:27 cvsmgr Exp $
...
Bool ColormapDefinition::loadBuiltinColormap(const String& name) {
  ...
  for (uInt i=0; i<names.nelements(); i++) {
    if (name == names(i)) {
      itsReds.resize(2);itsGreens.resize(2);itsBlues.resize(2);
      itsReds(0)= 0.0;itsReds(1)= 1.0;
      itsGreens(0)= 0.0;itsGreens(1)= 1.0;
      itsBlues(0)= 0.0;itsBlues(1)= 1.0;
      return True;
    }
  }
  return False;
...
Bool ColormapDefinition::queryColormapTable(const Table& table, 
  ...
  if (foundMatch) {
    ...
    itsReds.resize(n);itsGreens.resize(n);itsBlues.resize(n);

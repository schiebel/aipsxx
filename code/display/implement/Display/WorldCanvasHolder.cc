...
//#
//# $Id: WorldCanvasHolder.cc,v 19.8 2005/06/15 17:56:34 cvsmgr Exp $
...
void WorldCanvasHolder::operator()(const WCRefreshEvent &ev) {
  ...
  for(dds.toStart(),dd=0; !dds.atEnd(); dds++,dd++) {
    dData = dds.getRight();
    if(isCSmaster(dData) || conforms[dd]) {
	// removing this ^^^^^^^^^^^^^^^ part would be safer
	// (but somewhat less convenient for the user).
      if(dData->labelAxes(ev)) break;  }  }

...
//#
//# $Id: PixelCanvas.cc,v 19.6 2005/06/15 17:56:30 cvsmgr Exp $
			      ...
			      const Float &pangle, Bool outline)
{
  ...
  Float ang, temp;
  uInt nsegments = uInt(2 * C::pi * smajor + 0.5);
  if (nsegments < 4) {
    nsegments = 4;
  }
  ...
  for (uInt j = 0; j < nsegments; j++) {
    ...
    py(j) = sminor * sin(ang);
    ang = theta;
    temp = cx + px(j) * cos(ang) - py(j) * sin(ang);
    py(j) = cy + px(j) * sin(ang) + py(j) * cos(ang);
    px(j) = temp;
  }
//
  if (outline) {

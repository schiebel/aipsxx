...
//#
//# $Id: GLPCTexture.cc,v 19.4 2005/06/15 17:56:27 cvsmgr Exp $
#if defined(OGL)
#include <stdio.h>
...
class GLPCTextureObject {
  public:
	...
	void drawTexture();
	GLsizei tWidth()const{return twidth_;}
	GLsizei tHeight()const{return theight_;}
	GLsizei dWidth()const{return dwidth_;}
	GLsizei dHeight()const{return dheight_;}
	GLuint	textureid()const{return texid_;}
  public:
...
void GLPCTextureObject::drawTexture()
{
	...
	glBegin(GL_QUADS);
	 	glTexCoord2f(tx0, ty0); glVertex3f(x0, y0, z_);
	 	glTexCoord2f(tx0, ty1); glVertex3f(x0, y1, z_);
	 	glTexCoord2f(tx1, ty1); glVertex3f(x1, y1, z_);
	 	glTexCoord2f(tx1, ty0); glVertex3f(x1, y0, z_);
	glEnd();

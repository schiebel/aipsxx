...
//#
//# $Id: PrincipalAxesDD.cc,v 19.17 2006/09/04 04:17:08 mmarquar Exp $
...
: iAmRubbish(True),
  ...
  itsSpectralNotationFixed(False),
...
{
  return showPosition(world, itsShowWorldCoordinate, itsAbsolute,
		      displayAxesOnly);
}

String PrincipalAxesDD::showPosition(const Vector<Double> &world,
				     const Bool &showWorld,
				     const Bool &showAbs,
				     const Bool &displayAxesOnly) 
{
...
Bool PrincipalAxesDD::setActiveZIndex(Int zindex) {
...
Bool PrincipalAxesDD::sizeControl(WorldCanvasHolder &wch,
  ...
  // via setActiveZIndex() (or the deprecated findActiveImage()).  As a
  ...
  // just before returning from setActiveZIndex()).
  ...
  if ((itsOptionsAspect == "fixed world") &&
    ...
    drawXSize = max(1.0,
		    drawSize * fabs(worldAxisIncrements()(0)) * imageXSize);
    drawYSize = max(1.0,
		    drawSize * fabs(worldAxisIncrements()(1)) * imageYSize);
  } else if ( itsOptionsAspect == "flexible") {
    drawXSize = (Double)canvasDrawXSize;
    drawYSize = (Double)canvasDrawYSize;
  } else {
    // apply fixed-pixel scaling
...
Bool PrincipalAxesDD::findActiveImage(WorldCanvasHolder &wch) {
  ...
  // (setActiveZIndex() is now used everywhere instead, via conformsTo()).
  // setActiveImage (now setActiveZIndex) assumes that DDelement[i] will
...
// in the CS used for position tracking
  ...
  if (readOptionRecord(itsDoppler, error, rec, "velocitytype") ||
...
//
  ...
  if (readOptionRecord(value, error, rec,  "spectralnotation")) {
     itsSpectralNotationFixed = (value=="Fixed");
  }
//
...
//
     ...
     spectralnotation.define("listname", "Spectral value spectralnotation");   
     spectralnotation.define("ptype", "choice");
     Vector<String> choices(2);
     choices(0) = "Scientific";
     choices(1) = "Fixed";
     spectralnotation.define("popt", choices);
     spectralnotation.define("default", "Scientific");
     if (itsSpectralNotationFixed) {
       spectralnotation.define("value", "Fixed");
     } else {
       spectralnotation.define("value", "Scientific");
     }
...
Bool PrincipalAxesDD::conformsToCS(const WorldCanvasHolder& wch) {
  ...
  // WCH restrictions, and serves (via setActiveZIndex()) to inform the

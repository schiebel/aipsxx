...
//#
//# $Id: LatticePADMContour.cc,v 19.5 2005/06/15 18:00:43 cvsmgr Exp $
...
uInt LatticePADMContour<T>::dataDrawSelf(WorldCanvas *wCanvas,
  ...
  */
  ...
  Vector<Float> levels(lac->itsLevels.nelements());
  for (uInt i = 0; i < levels.nelements(); i++) {
    if (lac->itsType == "abs") {
      levels(i) = lac->itsLevels(i) * lac->itsScale;
    } else {
      levels(i) = lac->getDataMin() + lac->itsLevels(i) * lac->itsScale *
	(lac->getDataMax() - lac->getDataMin());
    }
  }
  ...
  try {
    ...
    if (maskMatrix.nelements() == datMatrix.nelements()) {
      ...
      wCanvas->drawContourMap(blc, trc, datMatrix, maskMatrix, levels,
			      usePixelEdges);
    } else {

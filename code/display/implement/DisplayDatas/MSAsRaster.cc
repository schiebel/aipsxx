...
//#
//# $Id: MSAsRaster.cc,v 19.21 2006/01/18 23:52:15 dking Exp $
...
MSAsRaster::MSAsRaster(const String msname): 
  ...
  itsMS = new MeasurementSet(msname,  TableLock(TableLock::AutoNoReadLocking),
  				      Table::Update);
  if (itsMS==0) throw AipsError("Cannot open Measurement Set.");
  if (itsMS->nrow()==0u) throw AipsError("Measurement Set contains no data.");
...
void MSAsRaster::initMSAR_() {
  ...
  uInt nl=uInt(NOTLOADED), nd=uInt(NODATA),
	...
	// (to stop a new compiler whine....  Historical note: this
  ...
  r[of] = .53f; g[of] = .48f; b[of] = .8f;	// (medium blue)
  r[nf] = .56f; g[nf] = .76f; b[nf] = .9f;	// (lighter blue)
  ...
  for(AxisLoc loc=X; loc<=SL0; loc++) {
	...
	if(msShape_[ndax]>1) {
	  ...
	  for(AxisLoc sloc=rloc+1; sloc<NLOCS; sloc++) {
	    axisOn_[sloc-1] = axisOn_[sloc];  }  // shift others up, and...
  ...
  if(msselValid_) {
    ...
    for(Axis ax=0; ax<NAXES; ax++) msSize *= msShape_[ax];
...
void MSAsRaster::selectVS_() {
  ...
  if(vssel_!=0 && vssel_!=vs_) delete vssel_;
  vssel_=0;				// delete old selected VisSet and
  ...
  cerr<<"Done."<<endl;  }		// progress feedback
...
void MSAsRaster::findRanges_() {
  ...
  if(!msselValid_) {
    ...
    msShape_=1;		// Some routines expect the visibility shape
			// to be at least one pixel wide on each axis.
    pos_=0;
    if(itsVisMb!=0) itsVisMb->setDescription(visMbLabel_()); 
    nAnt_=1;
    time_.resize(1); time_=0.;
    field_.resize(1); field_=0;
    scan_.resize(1); scan_=0;
    fieldName_.resize(1); fieldName_="";
    resetFreq_();
    spwId_.resize(1, True); spwId_=0;
    freq_.resize(1, True); freq_[0] = new Vector<Double>(1, 0.);
    nPolIds_=1;
    nPolsIn_.resize(nPolIds_); nPolsIn_=1;
    pidBase_.resize(nPolIds_); pidBase_=0;
    polId_.resize(1); polId_=0;
    polName_.resize(1); polName_="";
  ...
  pidBase_.resize(nPolIds_); pidBase_ = INVALID;
  ...
  for (vi.originChunks(); vi.moreChunks(); vi.nextChunk() ) {
	...
	spwId_[spw] = spwId_[sprev];  }   // larger than spwid up one
	  				  // slot, to make room.
    ...
    Double oldTime=-1.e15;  Int iTime=-1;
	...
	if(oldTime!=time) {
	  ...
	  for(Int i=nTime-1; i>iTime; i--) {
	    time_[i]=time_[i-1];     scan_[i]=scan_[i-1];
	    field_[i]=field_[i-1];   fieldName_[i]=fieldName_[i-1];  }

	  // Fill in the new information for this time slot.

	  time_[iTime] = time;      scan_[iTime]=vb.scan()[row];
	  field_[iTime]=vb.fieldId();  fieldName_[iTime]=vi.fieldName();
  ...
  msShape_[BASELN] = (nAnt_==1)?
		   nFeed :   // single antenna case
  		   bsln_(nAnt_-1, nAnt_-1) + 1;
			     // interferometer case: 1 more than final index.
  ...
  cerr<<"  Selected MS:  Time slots: "<<msShape_[TIME]<<
	((nAnt_==1)?
	"  Feeds: " :
	"  Baselines (incl. gaps): ")<<msShape_[BASELN]<<endl<<
	"  Polarizations: "<<msShape_[POL]<<
  ...
  // Baseline is more usefully reset to baseline 1-2, rather than 1-1,
  // for interferometric data.
  
  if((pos_[BASELN] = mspos_[BASELN]) == INVALID) {
    pos_[BASELN] = (nAnt_!=1 && msShape_[BASELN]>1)? 1 : 0;  }  }
...
void MSAsRaster::constructParameters_() {
  ...
  visCompName_(REAL)="Real"; visCompName_(IMAGINARY)="Imaginary";
  ...
  itsFlagColor = new DParameterChoice("flagcolor",
     ...
     maskClr, maskClr(1), maskClr(1), "Flagging_Options");
...
void MSAsRaster::setCS_() {
  ...
  if(nAnt_==1) cs.addCoordinate(lc);	// (no baselines, just feeds).
    ...
    if(axX==BASELN || axY==BASELN) {
      ...
      Vector<Double>pix(2*nAnt_ + 3), wld(2*nAnt_ + 2);
...
Record MSAsRaster::getOptions() {
  ...
  itsXAxis->toRecord(rec); itsYAxis->toRecord(rec); itsZAxis->toRecord(rec);
  ...
  itsSL0Pos->toRecord(rec);  itsSL1Pos->toRecord(rec);
  ...
  flgall.define("value", "");
	// (Note: above should be changed to _current_ value, if getOptions
	//  is ever needed for anything besides initialization...)
  ...
  itsDataMin->toRecord(rec); itsDataMax->toRecord(rec);
...
Bool MSAsRaster::setOptions(Record &rec, Record &recOut) {
  ...
  Block<Int> uiAxisOn(3);  Block<Bool> swapAxes(3, False);
  ...
  if(axisChg) {
    ...
    for(AxisLoc newloc=X; newloc<=Z; newloc++) if(swapAxes[newloc]) {
      ...
      AxisLoc oldloc=X; while(axisOn_[oldloc]!=newax) oldloc++;
      axisOn_[oldloc]=oldax; axisOn_[newloc]=newax;  }
  ...
  Bool newRanges = False;	// indicates MS selection change.
  Bool notfound;
  ...
  if(!notfound) {
    ...
    for(Int f = uiBase(); f < nFieldIds_+uiBase(); f++) {
      for(Int i=0; i<nInputFlds; i++) if(f==inputFieldIds[i]) {
        newFieldIds[nNewFlds++] = f;  break;  }  }
	
    newFieldIds.resize(nNewFlds, True);  }
  ...
  if(!notfound) {
    ...
    for(Int s=uiBase(); s<nSpwIds_+uiBase(); s++) {
      for(Int i=0; i<nInputSpws; i++) if(s==inputSpwIds[i]) {
        newSpwIds[nNewSpws++] = s;  break;  }  }
	
    newSpwIds.resize(nNewSpws, True);  }
  ...
  // resetting is necessary after a change to MS selection (ranges).  
  ...
  // Changes to X or Y axes or to the axis ranges require 
  // RESETTING THE COORDINATE SYSTEM.

  if(dspAxisChg || newRanges) setCS_();
  
  
   
 
  // SEND SLICE POSITIONS AND RANGES OUT TO ANIMATOR AND SLIDER controls,
  // (synchronizing them with final values of the pos_ variable).
  ...
  if( isCSmaster() &&
      (axZChg || rngZChg || (newRanges && pos_[axZ]!=oldPosZ)) ) {
    Record setanimrec;
 ...
 Bool applyPressed = (rec.nfields()>1);  // More than one option was
  ...
  if( msselValid_ &&
      (applyPressed || (visDataChg_ && curVisComp_!=INVALID_VC)) ) {
	// Reload of data from disk will automatically occur if
	// visType_ or visComp_ is changed (except the first time
	// on a really large MS).
	...
	if(pos_[axSL0]>=visStart_[axSL0] &&
	  ...
	  Block<Int> visShp(NAXES); computeVisShape_(visShp);
		// a 'tentative' visShape_: what visShape_ will be if we
		// have to call extract_.
  ...
  if(visValid_ && (postDataRng_ || wasDev!=isDev)) {
    ...
    needsRefresh = True;
    postDataRng_ = False;  }	// (msg. flag has been acted on)
    ...
    if(visValid_) {
      ...
      if(isDev) {
         dmin=devRngMin_; dmax=devRngMax_;  }
      newrgmin = min(newrgmin, dmin);		// expansion due to
      newrgmax = max(newrgmax, dmax);  }	// computed data ranges.
  ...
  //#dk  (To Do: implement a DParameter for these checkboxes instead).
...
Bool MSAsRaster::draw_(Display::RefreshReason reason,
  ...
  if(dispFlagsInClr_ && !flagsInClr_) {
    ...
    // flag display.
    Int nx=dispMask_.shape()(X); Int ny=dispMask_.shape()(Y);
  ...
  linToWorld(wblc, lblc); linToWorld(wtrc, ltrc);
...
void MSAsRaster::extract_() {
  ...
  //	     curVisComp_, visShape_, visStart_), which has the requested
  //	     visType_ and visComp_, and includes the requested display
  //         slice(s).
  ...
  vis_.resize(IPosition(5,   vnTime, vnBsln, vnChan, vnPol, vnSpw));
  ...
  // vis_ will be the block of data that includes requested slice and
  ...
  if(phase) { dataRngMin_=-180.; dataRngMax_=180.;  }
	// phase range is hard-coded (is this a good idea?...)
  else if(visDataChg_) { dataRngMin_=0; dataRngMax_=0;  } // reset ranges.
  Double sumv=0., sumv2=0.; Int nvis=0;
  ...
  Cube<Complex> vc, vc0;
  Cube<Bool> flg;  Vector<Bool>flgRow;
  ...
  for (vi.originChunks(); vi.moreChunks(); vi.nextChunk()) {
    ...
    for (vi.origin(); vi.more(); vi++) {
      ...
      if(resid) {
	vi.visibility(vc, VisIter::Corrected);
	vi.visibility(vc0, VisIter::Model);  }
		// need both corrected and model vis cubes for residual.
      else
	vi.visibility(vc, (VisIter::DataColumn)visType_);
	...
	while(time!=searchTime) {	// we should find an exact match...
	  ...
	  if(iTime>=veTime) {		// ... unless we're beyond time
	    ...
	    break;  }			// we're done with the whole chunk.
	...
	// values.  This is the code dealing with 'Offset', 'fslot' and
        ...
        if(newTime) {
	  tsOffset = vnBsln*(vnSpw*(iTime-vsTime) + spw-vsSpw);
	  newTime=False;  }
	...
	for (Int pol=frstPol; pol<lstPol; pol++) {
	  ...
	  for (Int chn=frstChan; chn<lstChan; chn++) {
	    ...
	    if(flgRow(row) || flg(vcpol,chn,row)) {	// store flags
	      ...
	      sumv += v; sumv2 += Double(v)*v; nvis++;  }  }  }  }


      iIter++;
      if(doneChunk) break;	// (from vi minor iterations).
      iRow+=nRow;  }		// for(vi minor iterations)

    iRow = iRowChunk + vi.nRowChunk();
    iChunk++;  }		// for(vi Chunk iterations)
  ...
  purgeCache();  }	// Assures that displayed data corresponds to
...
void MSAsRaster::createDisplaySlice_() {
  ...
  dispDevValid_ = False;  }	// deviation Matrix is _not_ yet up-to-date.
...
void MSAsRaster::handleEvent(DisplayEvent& ev) {
  ...
  if(chev != 0) {
    ...
    pix(0) = chev->pixX(); pix(1) = chev->pixY();
  ...
  if(rev != 0) {
    ...
    for(Int i=0; i<2; i++) {
      pixb(i)=rev->pixBlc()(i); pixt(i)=rev->pixTrc()(i);  }
    wc->pixToLin(linb, pixb); wc->pixToLin(lint, pixt);
    ...
    if(ix0<ix1 && iy0<iy1) {
      ...
      ResetRTRegionEvent ev;
      wc->handleEvent(ev);  };	// (erase rectangle, and you refresh).
...
void MSAsRaster::postEditsToDisp_() {
  ...
  for(edits.toStart(); !edits.atEnd(); i++, edits++) {
    ...
    for(Axis ax=0; ax<NAXES; ax++) {
      ...
      if(!edit.appliesTo(ax, dispPos_[ax])) {
        applies=False; break;  }  }  // edit doesn't apply to this display
      ...
      if(!dispFlagsInClr_) {
        for(Int ix=startX; ix<endX; ix++) if(edit.applies2(dispX_, ix)) {
          for(Int iy=startY; iy<endY; iy++) if(edit.applies2(dispY_, iy)) {
            dispMask_(ix,iy) = False;  }  }  }	// mask out the pixel...
      else {
        for(Int ix=startX; ix<endX; ix++) if(edit.applies2(dispX_, ix)) {
          for(Int iy=startY; iy<endY; iy++) if(edit.applies2(dispY_, iy)) {
  ...
  dispNEdits_ = flagEdits_.len();  }  // disp_ is up-to-date with edits now.
...
Bool MSAsRaster::saveEdits_() {
  ...
  for(edits.toStart(); !edits.atEnd(); edits++) {
    ...
    if(edit.all[TIME]) {
      ...
      Int bs, be; edit.getLoopRange(BASELN, bs,be);
      for(Int b=bs; b<be; b++) if(edit.appliesTo(b)) usedallt[b]=True;  }
    
    else {
      Int ts, te; edit.getSureRange(TIME, ts,te);
  ...
  if(!alltused) {		// (maxTime will be used only in this case).
    ...
    if(tmax<0) { undoEdits_("all"); return False;  }	// (shouldn't happen)
    maxTime=time_[tmax];  }
  ...
  VisSet* vs = (entireMS)? vs_ : vssel_;  // Iterate either entire MS or
  VisIter& vi(vs->iter());		  // selected MS, acc. to user choice.
  ...
  for (vi.originChunks(); vi.moreChunks(); vi.nextChunk() ) {
    ...
    for (vi.origin(); vi.more(); vi++) {
	  ...
	  tmr.mark(); intvl++;  }		// progress feedback
	...
	Double time = vb.time()[row];
	if(!alltused && time>maxTime) { doneChunk=True; break;  }
	...
	vi.setFlagRow(flgRow);  }
      ...
      if(doneChunk) break;  }		// ( ...out of vi minor iterations).

    iRow = iRowChunk + vi.nRowChunk();  }	// for(vi Chunk iterations)


  vs->flush();				// make sure flags go out to disk.
  mssel_->relinquishAutoLocks(True);	// (just to be sure).
  itsMS->relinquishAutoLocks(True);	// (just to be sure).
...
void MSAsRaster::computeTimeBoxcars_() {
  ...
  lsvTime_.resize(nvTimes); levTime_.resize(nvTimes);
...
Float MSAsRaster::dev_(Int t) {
  ...
  Float vt = v_(t); if(vt<=NO_DATA) return vt;
  ...
  if(useVis_) {
    st = lsvTime_[t]; if(st==sT_ && visDev_==RMS) return d_;
    et = levTime_[t];  }
  else {
    st = lsTime_[t];  if(st==sT_ && visDev_==RMS) return d_;
    et = leTime_[t];  }
  ...
  if(sT_>=0 && eT_-st > st-sT_ + 1) {
    ...
    nValid_ = 1;
    sumv_=vt; sumva_=vta;
    sumv2_=Double(vt)*vt; sumv2a_=Double(vta)*vta;
  ...
  if(nValid_<2) d_ = INSUF_DATA;	// not enough valid data in nbhd.
    ...
    if(sumdev2<0) {
      ...
      sT_ = -1;  }		// (shouldn't happen, except perhaps
...
void MSAsRaster::computeDevRange_() {
  ...
  Double sumdev=0., sumdev2=0.; Int ndev=0;
  ...
  if(visSize<100000.) {
    ...
    for(Int bsl=0; bsl<vnBsln; bsl++) {
      ...
      if(nAnt_!=1) {
        Double ant1, ant2;  a1a2_(ant1,ant2, bsl+visStart_[BASELN]);
    ...
    for(Int i=0; i<15000; i++) {
      ...
      Int bsl=randB.asInt();
      if(nAnt_!=1) {
        Double ant1, ant2;  a1a2_(ant1,ant2, bsl);
        if(ant2>=nAnt_ || (ant1==ant2 && !dish_))  continue;  }
	// skips baseline 'gaps' and autocorrs, if not single-dish.

      dPos_[BASELN]=bsl;         dPos_[CHAN]=randC.asInt();
      dPos_[POL]=randP.asInt();  dPos_[SP_W]=randS.asInt();
      Int t=randT.asInt();
      sT_=-1;
...
void MSAsRaster::createDevSlice_() {
  ...
  if(dispNotLoaded_) dispDev_ = NOT_LOADED;
    ...
    if(!useVis_) {
      ...
      if(dispX_ == TIME) {
        axlTm_=X; vsTime=vsX; veTime=veX;	// time axis is on X
        axlOther=Y; vsOth=vsY; veOth=veY;  }	// (other display axis is
      else {					//  on Y)
        axlTm_ = Y; vsTime=vsY; veTime=veY;	// time axis is on Y
	axlOther = X; vsOth=vsX; veOth=veX;  }
...
String MSAsRaster::showValue(const Vector<Double> &world) {
  ...
  pos[axisOn_[Z]] = activeZIndex_;	// Insert latest canvas animator
    ...
    // pos_[axisOn_(Z)].  setActiveZIndex(zindex) must be called prior
  ...
  if(visDev_>NORMAL) {
    ...
    Double sumv=v, sumv2=Double(v)*v; Int nvalid=1;
    ...
    Bool phase = (visComp_==PHASE);
    if(phase) { va = vAlt_(v); sumva=va; sumv2a=Double(va)*va;  }
	...
	pos[TIME] = tbox + visStart_[TIME];
      ...
      if(!fbox) {	// unflagged, valid data for the running average.
        nvalid++;
	sumv+=vbox; sumv2+=Double(vbox)*vbox;

	if(phase) {
          Float vboxa = vAlt_(vbox);
	  sumva+=vboxa; sumv2a+=Double(vboxa)*vboxa;  }  }  }
  ...
  ostringstream os; os << setprecision(4);
...
String MSAsRaster::showPosition(const Vector<Double> &world,
  ...
  ostringstream os; os << setprecision(5);
  ...
  if(validt) {
    ...
    os<<"Scan "<<scan_[pt];  }  // (scan #s always reported as set in MS).
  os<<endl;
  startp=os.tellp();
  ...
  if(szb>1) {
    ...
    if(nAnt_==1) {
      ...
      if(!validb) os<<"Invalid ";
        ...
        Double a1, a2;
        a1a2_(a1,a2, pb, uiBase());
        os<<setw(4)<<right<<a1;
        if(a2>nAnt_-1+uiBase()) os<<"|"<<setw(3)<<left<<a1+1;
		// "Dividing line" between antennas (no data here)
        else                    os<<"-"<<setw(3)<<left<<a2;  }
		// Real baseline
      os<<"(b "<<pb+uiBase()<<")";  }  }
...
void MSAsRaster::computeVisShape_(Block<Int>& visShp) {
  // Compute vis_ array dimensions which fit into allowed memory.
  // vis_ is a 'cursor' or 'window' into the whole selected, gridded ms
  // whose dimensions are given by msShape_.  It _will_ be the entire
  // hypercube if it fits into memory; in any case, the two display
  // axes will be full size.

  Double kbMS=sizeof(Float)/1024.;
  for(Axis ax=0; ax<NAXES; ax++) kbMS *= msShape_[ax];
  Double kbVis=itsVisMb->value() * 1024.;  // max Mb, from user interface.
  kbVis = max(kbVis, 5.*1024.);
  Double fctr=kbMS/kbVis;

  visShp= msShape_;		// Trim from here, if necessary.

  // Trim the length of the memory buffer axes as necessary, to keep vis_
  ...
  for(Int i=0; i<NAXES && fctr>1.; i++) {
    ...
    visShp[tax] = Int(max(1.,visShp[tax]/fctr));
    fctr = fctr*visShp[tax]/msShape_[tax];  }
  if(fctr>1.) visShp[axisOn_[Z]] = Int(max(1.,visShp[axisOn_[Z]]/fctr));  }
...
Bool MSAsRaster::undoEdits_(String extent, Bool feedback) {
  ...
  while(!edits.atStart()) {
    ...
    edits.removeRight();
    if(extent!="all") { ndeleted=1; break;  }  }
...
Int MSAsRaster::nframes_(const Block<Int>& frames, Int strtfrm, Int nfrms,
  ...
  for(Int i=0; i<tfrms; i++) {
    Int frm=frames[i]; if(frm>=endfrm) break;
    if(frm>=strtfrm) {
      nframes++;
...
Int MSAsRaster::maxframes_(const Block<Int>& frames,
  ...
  strtfrm=0;
  Int tfrms=frames.nelements();  if(tfrms==0 || nfrms<1) return 0;

  strtfrm=-1; Int maxframes=-1;  // (Will be immediately overridden...)
  Bool multiple=False; Int margin=-1;
...
void MSAsRaster::setDefaultOptions() {
  ...
  setDefaultMSAROptions_();  }              // (this level)
...
String MSAsRaster::visMbLabel_() {
  ...
  for(Axis ax=0; ax<NAXES; ax++) mbVis *= msShape_[ax];
...
const Unit MSAsRaster::dataUnit() {
  // for position tracking--Unit of the displayed data values
  switch(curVisComp_) {
    case PHASE:		return "deg";
    case INVALID_VC:	return "";
    default:		return "Jy";  }  }
...
void MSAsRaster::setFlag_(IPosition& slot, Bool flag) {
  ...
  else     flags_(indx) &= ~(1u<<bit);  }	// Reset flag
...
Bool MSAsRaster::FlagEdit_::appliesTo(Int bsln) {
  ...
  // include one of the edit's selected antenna[s]?
  // The 'antenna[s] to edit' are only the _ant1[s]_ of the baseline
  // region chosen with the mouse.  (In most cases, the user will select
  // just a single antenna to edit, by clicking the crosshair somewhere in
  // its 'antenna1-block').  However, any baseline will be [un]flagged which
  // has one of these 'antenna[s] to edit' as _either_ its ant1 or ant2.

  Int strt, fin; getSureRange(BASELN, strt,fin);
  Double firstAnt, lastAnt;  // the user-selected antenna[s] to edit.
  Double a1, a2;
  Double firstBsl=strt;
  Double lastBsl=fin-1.;     // (we want _inclusive_ final baseline here...)
  msar->a1a2_(firstAnt, a2,  firstBsl);
  msar->a1a2_(lastAnt, a2,  lastBsl);	   // (...or its antenna1, actually).
  msar->a1a2_(a1, a2,  bsln);	// antennas of the baseline being tested.

  return (a1>=firstAnt && a1<=lastAnt  ||
	  a2>=firstAnt && a2<=lastAnt);  }
...
Bool MSAsRaster::FlagEdit_::appliesTo(Double time) {
  ...
  Int st, et; getSureRange(TIME, st, et);
...
Int MSAsRaster::MSpos_::t() {
  ...
  return INVALID;  }	// bracketing time slots in this case).
...
Int MSAsRaster::MSpos_::p() {
  Int ps = p0(); if(ps==INVALID) return INVALID;
...
void MSAsRaster::resetMinMax_() {
  ...
  Float dmin=dataRngMin_, dmax=dataRngMax_;
  if(visDev_>NORMAL) { dmin=devRngMin_; dmax=devRngMax_;  }

  *itsDataMin = dmin;           *itsDataMax = dmax;
  itsDataMin->setMinimum(dmin); itsDataMax->setMinimum(dmin);
  itsDataMin->setMaximum(dmax); itsDataMax->setMaximum(dmax);  }
...
MSAsRaster::~MSAsRaster() {
  ...
  if(msValid_) deleteParameters_();
  removeColormap();
  resetFreq_();

  Bool printwarning=True;
  undoEdits_("all", printwarning);
	// delete any leftover flagging edits--user will lose them.
  
  if(vssel_!=0 && vssel_!=vs_) delete vssel_;	// delete old VisSets,...
  if(vs_!=0) delete vs_;
  if(mssel_!=itsMS && mssel_!=0) {
    mssel_->relinquishAutoLocks(True);
    delete mssel_;  }			// selected MS,...
  if(msCols_!=0) delete msCols_;
  if(itsMS!=0) {
...
void MSAsRaster::deleteParameters_() {
  ...
  delete itsAxisLabelling; delete itsFlagColor; delete itsUnflag;
  delete itsEntireAnt; delete itsUndoOne; delete itsUndoAll;
  delete itsEditEntireMS; delete itsSaveEdits;  }

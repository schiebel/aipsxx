...
//#
//# $Id: ScrollingRasterDD.cc,v 19.8 2005/06/15 18:00:46 cvsmgr Exp $
//
...
ScrollingRasterDD::~ScrollingRasterDD() {
  for (uInt i=0; i<nelements(); i++) {
    if(DDelement[i]!=0) {
      delete static_cast<ScrollingRasterDM*>(DDelement[i]); DDelement[i] = 0; // always :>
    }
  }

  delete itsResampleHandler; itsResampleHandler = 0;
  if (itsLattices) {
    for (uInt i=0; i<itsScanNumber; i++) {
      if (itsLattices[i]) {
        delete itsLattices[i]; itsLattices[i] = 0;
      }
    }
    delete[] itsLattices; itsLattices = 0;
  }
  delete itsLatticeStatisticsPtr; itsLatticeStatisticsPtr = 0;
  delete itsFilledDisplayedLatticePtr; itsFilledDisplayedLatticePtr = 0;

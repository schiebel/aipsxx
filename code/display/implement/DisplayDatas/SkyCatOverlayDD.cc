...
//#
//# $Id: SkyCatOverlayDD.cc,v 19.6 2005/06/15 18:00:46 cvsmgr Exp $
...
void SkyCatOverlayDD::constructParameters() {
  ...
  Vector<String> vstring3;
  Vector<String> vstring4;
  vstring3 = getColumnNamesOfType(TpFloat);
  if ( vstring3.nelements() > 0 ) {
    k=0;
    for (uInt i=0; i< vstring3.nelements(); i++) {
      // exclude direction type column since it is not useful
      if (vstring3(i) != itsLongitudeColumn && 
	  vstring3(i) != itsLatitudeColumn) {
	vstring4.resize(k+1,True);
	vstring4(k) = vstring3(i);
	k++;
      }
    }
  }

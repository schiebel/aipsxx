...
//#
//#
//# $Id: MSAsRaster.h,v 19.12 2006/01/18 23:52:15 dking Exp $
...
class MSAsRaster: public ActiveCaching2dDD {
 ...
 public:
  ...
  virtual Bool setActiveZIndex(Int zindex) {
	...
	// activeZIndex_ and zIndexConformed_ protected on the DD base level;
	// they are intended to be set only by these setActiveZIndex() methods.
 ...
 private:
  ...
  // return baseline index (in the usual baseline ordering) from antenna
  // numbers.  Input must have 0 <= a1 <= a2 < nAnt_.
  ...
  // Double return value returns a1*a1mult_() + a2.
  // <group>
  ...
  // places are needed for an antenna number.  Returns 1000, unless 4
  // places are needed, in which case it returns 10000, etc.
  Double a1mult_() { Double m=1000.; while(m<=nAnt_) m*=10.; return m;  }
...
//===================    Data    ======================= (mostly) ========
 ...
 //-----more constants and statics ----------------------------------------
  ...
  Block<Int> visShape_;  // shape of extracted vis_ Array and
  ...
  struct FlagEdit_ {
    ...
    // This holds information about an individual new flagging edit command.
    ...
    Block<Int> shape;   // (but not including) start[axis]+shape[axis].
			...
			// shape[axis]==msShape[axis], but _not vice-versa_.

...
//#
//# $Id: ActiveCaching2dDD.cc,v 19.10 2006/01/19 00:10:22 dking Exp $
...
Bool ActiveCaching2dDD::sizeControl(WorldCanvasHolder &wch,
  ...
  if (reset || unzoom) {
    linXMin = itsPixelBlc[0];  linYMin = itsPixelBlc[1];
    linXMax = itsPixelTrc[0];  linYMax = itsPixelTrc[1];  }
  ...
  if(!reset &&  wCanvas->existsAttribute(zoomB) &&
    ...
    if (zoomBlc.nelements() >= 2 && zoomTrc.nelements() >= 2) {
      linXMin = zoomBlc[0]; linXMax = zoomTrc[0];
  ...
  mn=floor(linXMin+.5); mx=floor(linXMax+.5);
  if(mn==mx) ctr=mn; else ctr=(linXMin+linXMax)/2.;
  linXMin = max(itsPixelBlc[0], min(itsPixelTrc[0]-1., min(ctr-.5, linXMin)));
  linXMax = max(linXMin+1.,     min(itsPixelTrc[0],    max(ctr+.5, linXMax)));

  mn=floor(linYMin+.5); mx=floor(linYMax+.5);
  if(mn==mx) ctr=mn; else ctr=(linYMin+linYMax)/2.;

...
//#
//# $Id: WCCSNLAxisLabeller.cc,v 19.11 2006/01/18 23:51:34 dking Exp $
...
//
  ...
  uInt ididx;
  strncpy(idents + 0, xAxisText().chars(), nl);
  for (ididx = xAxisText().length(); ididx < nl; ididx++) {
    idents[0 + ididx] = 32;
  }
  strncpy(idents + nl, yAxisText().chars(), nl);
  for (ididx = yAxisText().length(); ididx < nl; ididx++) {
    idents[nl + ididx] = 32;
  }
...
// itsSpectralUnit
        ...
        tPix = cSys.referencePixel();
	String tStr;
        if (!cSys.toWorld(tWrld,tPix)) {
        } else {
           zLabel = cSys.format(tStr, Coordinate::DEFAULT, tWrld(2), 2);
        }
//
	zLabel += " ";
	zLabel += tStr;

...
//#
//# $Id: QtPixelCanvas.qo.h,v 1.6 2006/10/10 21:42:05 dking Exp $
...
//# RELEVANT (OR ALREADY-IMPLEMENTED) METHODS
  ...
  virtual void setDrawBuffer(Display::DrawBuffer buf) {
    ...
    p_begin_();  }	// Refocus painter on new buffer if necessary.
 ...
 // buffer memory exchanges
  ...
  virtual uInt width() const{ return uInt(QWidget::width());  }
  
  // Return the height of the PixelCanvas in pixels
  virtual uInt height() const { return uInt(QWidget::height());  }
  
  // Return the depth of the PixelCanvas in bits
  virtual uInt depth() const { return itspcctbl->depth();  }
  ...
  //#       MUST implement in future
  
  virtual void drawImage(const Int &x, const Int &y, 
                         const Matrix<uInt> &data,
			 const Matrix<Bool> &mask) ;
	//# The version used with masks (the one with the
	//# gratuitously transposed arguments...)
  ...
  virtual QPixmap* contents() { return new QPixmap(*frontBuffer_);  }
 ...
 private:
  ...
  // (converts either way).
  Int   q_(Int pY)   { return QWidget::height()-1 - pY;  }
  Float q_(Float pY) { return QWidget::height()-1 - pY;  }
  ...
  void qFromPC_(Int  x1, Int  y1,   Int x2,  Int y2,
    ...
    qw = abs(x2-x1)+1;  qh = abs(y2-y1)+1;
    qx = min(x1,x2);    qy = q_(max(y1,y2));  }
  ...
  Int clrByte_(Float f) { return Int(min( max(0.f,f)*256.f, 255.f ));  }

  Bool inDrawMode_() { return (drawMode()!=Display::Compile);  }
  
  QPixmap* currentBuffer_() {
    return (drawBuffer() == Display::FrontBuffer)?
		       frontBuffer_ : backBuffer_;  }
  
  void clearBuffer_() { p_.eraseRect(0,0, currentBuffer_()->width(),
 ...
 // </group>
  ...
  QPixmap* backBuffer_;		// of resize() require these to replaced
  				// often on the heap...).
...
//# (for 3-channel drawing)
  ...
  Display::ColorModel colorModel() const { return Display::Index;  }
...
//#diag cout << "drawImage uInt 2 not implemented" << endl;
};
...
//#        RELEVANCE TO BE DETERMINED
  ...
  virtual void drawColoredPoints(const Vector<Int> &x1, const Vector<Int> &y1,
     ...
     //#diag cout << "WYSISWYN " << __FILE__ << " " << __LINE__ << endl;
  }
  ...
  virtual Bool setFont(const String &fontName) { return False; }
  virtual Bool setFont(DLFont* font) { return False; }
  virtual Bool setFont(const String& fontName, const Int fontSize) {
    return False; }
  
//none of these can be kept virtural, it may be called by some dds 
//show something to remind if that case 

...
//#
//# $Id: QtDisplayPanel.cc,v 1.8 2006/10/10 21:42:05 dking Exp $
...
QtDisplayPanel::QtDisplayPanel(QtViewerBase* v, QWidget *parent) : 
 ...
 // Old-style 'connections' to underlying canvases.
...
void QtDisplayPanel::setOptions(Record rec) {
  ...
  try {
	...
	// call above may change the set of underlying canvases.
...
// WCMotionEH CALLBACK METHODS -- CURSOR POSITION TRACKING
...
void QtDisplayPanel::operator()(const WCMotionEvent& ev) {
  ...
  if(!myWC_(wc)) return;	// (safety; shouldn't happen)

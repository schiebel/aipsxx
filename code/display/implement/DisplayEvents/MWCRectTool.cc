...
//#
//# $Id: MWCRectTool.cc,v 19.5 2005/06/15 18:02:23 cvsmgr Exp $
...
void MWCRectTool::keyPressed(const WCPositionEvent &ev) {
  ...
  if (itsRectangleExists && ev.worldCanvas()==itsCurrentWC) {
    ...
    if(!itsEmitted) return;  }  // ignore click outside the rectangle
...
void MWCRectTool::moved(const WCMotionEvent &ev) {
  if (!itsActive) return;
...
void MWCRectTool::draw(const WCRefreshEvent &) {
  if(!itsRectangleExists) return;

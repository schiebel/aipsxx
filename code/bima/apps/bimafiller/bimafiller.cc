//# bimafiller:  miriad dataset to MeasurementSet filler
...
//#
//# $Id: bimafiller.cc,v 19.2 2004/11/30 17:50:11 ddebonis Exp $
//
...
//#include <../aips/mirlib/maxdimc.h>
//#include <../aips/mirlib/miriad.h>

#include <casa/namespace.h>

// MIRIAD dataset MeasurementSet filler. Although the name being
// bimafiller, it should be able to process general miriad data.
// The first release has only been tested with BIMA data, not ATCA.
//
		       ...
		       Bool Qtsys, Bool Qsplit, Bool Qarrays)
{
  ...
  if (Debug(1)) cout << "See also "
		     << "http://www.astro.umd.edu/~teuben/aips++/bimafiller.html" << endl;
  ...
  if (sizeof(int) != sizeof(Int))
    cout << "Double != double; bimafiller will probably fail" << endl;
		...
		// BAD CONVERSIONS !!!
  // we can't cast a String to a (char *), thus (const char *)             !!!!
  //  uvopen_c(&uv_handle_p, (const char *) infile_p, "old");		
  ...
  Tracking(-1);      // initialize those UV variables that need to be tracked
}
...
void BimaFiller::checkInput(Block<Int>& narrow, Block<Int>& wide, Block<Int>& window)
{
  ...
  for (;;) {
    ...
    object_p = vdata;
    if (Debug(1)) cout << "Source=>" << object_p << "<=" << endl;
...
void BimaFiller::fillAntennaTable()
{
  ...
  for (Int i=0; i<nAnt; i++) {
    ...
    ant.dishDiameter().put(row,diameter);   // all ants better be same
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    // Define inputs
    Input inp(1);
    inp.version("2 - BIMA to MS2 filler (22-oct-01) PJT)");
    inp.create("vis", "m51.test", "Name of BIMA dataset name", "string");    
    inp.create("ms", "m51.test.ms", "Name of MeasurementSet", "string");    
    ...
    Bool useTSM=inp.getBool("useTSM");
    //    Bool UseISM=inp.getBool("UseISM");

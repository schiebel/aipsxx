...
#
# $Id: tableutil.py,v 1.4 2006/09/26 04:28:23 gvandiep Exp $

from table import table
from table import _remove_prefix
from table import reverse_axes
...
def tablefromascii (tablename, asciifile,
    ...
    if headerfile != '':
        ...
        if not os.path.exists(filename):
            s = "File '%s' not found" % (filename)
            raise IOError(s)
    tab = table(tablename, asciifile, headerfile, autoheader,
                reverseaxes(autoshape),
                sep, commentmarker, firstline, lastline, _oper=1);
    print 'Input format: [' + tab._fromasciistring() +']';

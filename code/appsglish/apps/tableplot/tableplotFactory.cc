...
//#
//# $Id: tableplotFactory.cc,v 1.3 2005/04/05 20:41:34 rurvashi Exp $


#include <DOtableplot.h>
#include <tableplotFactory.h>
...
MethodResult tableplotFactory::make(ApplicationObject *&newObject,
			      ...
			      Bool runConstructor)
{
  ...
  if (whichConstructor == "tableplot") {
    
    //Parameter<String> msname(parameters, "msname", ParameterSet::In); 
    
    if (runConstructor) {
      newObject = new tableplot<float>();
      //newObject = new tableplot<float>(msname());
    }
  } 
   else {

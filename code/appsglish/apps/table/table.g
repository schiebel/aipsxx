...
#
#   $Id: table.g,v 19.7 2006/09/26 06:29:33 gvandiep Exp $
#
#----------------------------------------------------------------------------
...
# Create a new (named) table from a query of the sort "TIME > 54500.45";
  const public.query := function(query='', name='', sortlist='', columns='') {
    if(is_fail(public.ok())) fail;
    ...
    return tablecommand(command, self.handle);
  }

# Calculate an expression on the table.
  const public.calc := function(expr) {
...
const tablecommand := function(command, handle=F, tableserver=defaulttableserver) {
  local result;
  if (is_boolean(handle)) {

...
//#
//# $Id: newsimulatorFactory.cc,v 19.6 2005/11/07 21:17:04 wyoung Exp $
...
MethodResult newsimulatorFactory::make(ApplicationObject *&newObject,
			      ...
			      Bool runConstructor)
{
  ...
  if (whichConstructor == "newsimulator") {
    Parameter< String > msname(inputRecord, "msname",
			      ParameterSet::In);
    if (runConstructor) {
      newObject = new newsimulator(msname());
    }
  } else if (whichConstructor == "newsimulatorfromms") {
    Parameter< String > thems(inputRecord, "thems",
			      ParameterSet::In);
    if (runConstructor) {
      MeasurementSet thisms(thems(), Table::Update);
      newObject = new newsimulator(thisms);
    }
  } else {

# gpfframework.g: creates standard a standard gui frame for gainpolyfittergui
...
#
# $Id: gpfframework.g,v 19.1 2004/08/25 01:16:35 cvsmgr Exp $
#
# gpfframework: create a standard gui environment 
# (6/30/00) Same as guiframework, but with support for an optional
#  argument to action routines.
# (7/12/00) Added function to replace 'workframe'.
# (1/4/02) tweaked and renamed gpfframework.g 
#
#
...
#public.addactionhandler := function(aname, action, actionarg=F)
...
const gpfframework := function(title='AIPS++ Window',
			       menus=T, helpmenu=T, actions=T,
			       guiframenote=note, parent=F,
			       ws=dws)
{
   if(!have_gui()){
     fail('myguiframework makes no sense without a gui!');
   }
   ...
   # Call an action routine, with an arg if there is one. Returns the result
   # of the function. Caller is responsible for ensuring that the action
   # field exists and is a function.
   priv.callaction := function(ref rec, state=F) {
       if(has_field(rec, 'actionarg')) {
	   return rec.action(rec.actionarg);
       }
       else if (has_field(rec, 'actionstate') && rec.actionstate) {
	   return rec.action(state);
       }
       else
	   return rec.action();
   }
...
#
	   ...
	   local thevalue := pickone;
	   if (has_field(themenu[pickone], 'value')) 
	       thevalue := themenu[pickone].value;
	   local tag := spaste(mname,pickone);
           app.mb.btns[tag] := ws.button(app.mb.btns[mname],
                                    text=themenu[pickone].text, relief=brelief,
                                    type=btype, disabled=bdisabled,
                                    value=[menu=mname, item=pickone, 
					   value=thevalue]);
	   if (! is_agent(app.mb.btns[tag])) {
	       print 'button creation failed:', app.mb.btns[tag];
	       next;
	   }
#	   if(!has_field(themenu[pickone], 'text')) {
#	       print 'mname/pickone', mname, pickone;
#	       print themenu[pickone];
#	       print full_type_name(themenu[pickone]);
#	   }
	   ...
	   if (has_field(themenu[pickone], 'background')) 
	       app.mb.btns[tag]->background(themenu[pickone].background);
	       ...
	       local idx := spaste(mname,pickone);
	       public.handle[idx] := [=];
	       public.handle[idx].action := themenu[pickone].action;
	       if(has_field(themenu[pickone], 'actionarg')) {
		   public.handle[idx].actionarg :=
		       ref themenu[pickone].actionarg;
	       }
	       else if (has_field(themenu[pickone], 'actionstate') &&
			themenu[pickone].actionstate)
	       {
		   public.handle[idx].actionstate := T;
	       }
		   ...
		   local state := $value;
		   local which := spaste(state.menu, state.item);
		   state.state := app.mb.btns[which]->state();
		   if(has_field(public.handle, which) &&
		      is_function(public.handle[which].action)) {
		       public.handle::value := 
			   priv.callaction(public.handle[which],state);
		       public.handle::returned := T;
		       if(is_agent(public.handle::value)){
			   whenever public.handle::value->returns do {
			       public.handle.agent->returns(which);
			   }
		       }
		   }
	       }
   ...
   # Return the state of a button from a given menu.  This function will 
   # always return F unles the button is of a radio or check type
   public.getbuttonstate := function(menu, name) {
       wider app;
       local which := spaste(spaste(menu),name);
       if (! has_field(app.mb.btns, which))
	   fail paste('no such button on', menu, 'menu:', name);
       return app.mb.btns[which]->state();
   }

   # set the state of a button from a given menu.  This function will 
   # always return F unles the button is of a radio or check type
   public.setbuttonstate := function(menu, name, state=T) {
       wider app;
       local which := spaste(spaste(menu),name);
       if (! has_field(app.mb.btns, which))
	   fail paste('no such button on', menu, 'menu:', name);
       local ok := app.mb.btns[which]->state(state);
       return (ok == state);
   }

   # return the names of the buttons in a specified menu
   public.getbuttonnames := function(menu) {
       wider app;
       menu := spaste(menu);
       global GPFF_bnames := field_names(app.mb.btns);
       local names := eval(spaste('GPFF_bnames[GPFF_bnames ~ m/^', 
				  menu, '/];'));
       names =~ eval(spaste('s/^', menu, '//g'));
       names := names[strlen(names) > 0];
       return names;
   }
   ...
   { wider app;
     ...
     if(is_record(actions)){
		...
		if(has_field(actions[bname], 'actionarg')) {
		    public.addactionhandler(bname, actions[bname].action,
					    actions[bname].actionarg);
		}
		else {
		    public.addactionhandler(bname, actions[bname].action);
		}
   ...
   priv.initialize := function(windowtitle, menus, helpmenu, actions, ws)
   {  wider app;
      wider public;
      wider priv;
   ...
   # (6/30/00) Added optional actionarg
   public.addactionhandler := function(aname, action, ref actionarg=F)
   {  wider app;
      wider public;
      ...
      if(is_function(action)){
         public.handle[aname].action := action;
	 if(!missing()[3]) {
	     public.handle[aname].actionarg := ref actionarg;
	  }
      } else {
         #
         # Default action is to return the value of the button pressed.
         #
         public.handle[aname].action := F;
      }
      if(needs_whenever){
         whenever app.cmd.b[aname]->press do {
            public.handle::bpressed := $value;
            if(is_function(public.handle[$value].action)){
               public.handle::value :=
		   priv.callaction(public.handle[$value]);
            } else {
   ...
   public.app := priv.initialize(title, menus, helpmenu, actions,ws=ws);
   return ref public;

}
# const gpfgf := gainpolyfitter_framework;

# dishsmoothgui.g: dish smoother GUI
#------------------------------------------------------------------------------
...
#
#    $Id: dishsmoothgui.g,v 19.1 2004/08/25 01:11:35 cvsmgr Exp $
#------------------------------------------------------------------------------
...
const dishsmoothgui := subsequence(parent, itsdish, logcommand, widgetset=dws)
{
    ...
    private.op := itsdish.ops().smooth;
    private.logcommand := logcommand;

  private.HANNING := 0;
  private.BOXCAR := 1;
  private.GAUSSIAN := 2;
  private.maxtype := private.GAUSSIAN;
  ...
  private.label := dws.label(private.outerFrame, 'Smoothing');
  ...
  popuphelp(private.doSmoothButton,
	    hlp='Do the indicated smoothing');
  ...
  self.setdecimate := function(tOrF=F) {
	...
	private.logcommand('dish.ops().smooth.setdecimate',[tOrF=tOrF]);
	return T;
  }
...
#  self.setgausswidth:=function(gausswidth,units=private.GWCHANNELUNITS){
   self.setgausswidth:=function(gausswidth) {
        wider private;
	ok:=private.gwidthEntry->delete('start','end');
        ok:=private.gwidthEntry->insert(as_string(gausswidth));
	private.logcommand('dish.ops().smooth.setgausswidth',
			[gausswidth=gausswidth,units=private.gwcurrUnits]);
        return T;
  }
  ...
  self.setboxwidth:=function(boxwidth){
	...
	private.logcommand('dish.ops().smooth.setboxwidth',[boxwidth=boxwidth]);
	return T;
  }
  ...
  self.settype := function(whichType) {
	...
	private.bwidthEntry.disabledAppearance(T);
	private.gwidthAppearance(T);
        private.logcommand('dish.ops().smooth.settype',[type='HANNING']);

      } else if (private.currType == private.BOXCAR) {
	private.boxcarType->state(T);
	private.bwidthEntry.disabledAppearance(F); 
	private.gwidthAppearance(T);
        private.logcommand('dish.ops().smooth.settype', [type='BOXCAR']);
      } else {
	private.gaussianType->state(T);
        private.bwidthEntry.disabledAppearance(T);
        private.gwidthAppearance(F);
	private.logcommand('dish.ops().smooth.settype', [type='GAUSSIAN']);
      }
      if (private.currType == private.GAUSSIAN) {
  ...
  whenever private.hanningType->press, private.boxcarType->press, private.gaussianType->press do {
      wider private;
      self.settype($value);
  }

#  whenever private.bwidthEntry->return do {
#	wider private;
#	self.setboxwidth(private.bwidthEntry->get());
#  }

#  whenever private.gwidthEntry->return do {
#        wider private;
#        self.setgausswidth(private.gwidthEntry->get());
#  }
  ...
  private.smoother := itsdish.ops().smooth;
  self.smoother := function() { wider private; return private.smoother;}
  ...
  private.convertGWUnits := function() {
	...
	if (is_sdrecord(nominee)) {
		...
		# to X-axis units
		currWidth *:= nominee.data.desc.chan_width;
  ...
  private.gwidthConverterWidget.setunitcallback(private.setGWUnits);
	...
	wider private;
	private.op.apply();
	private.logcommand('dish.ops().smooth.apply');
    }

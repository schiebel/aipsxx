# map: synthesis integration one level higher than imager and calibrater
...
#
# $Id: map.g,v 19.1 2004/08/25 01:25:06 cvsmgr Exp $
...
#
const _define_map := function(msfile) {
#
   private := [=]
   public := [=]
#
                  ...
                  # CATALOG
                     ...
                     if (catalogname != 'Perley-Taylor 95') {
                        note('Only Perley-Tayor 95 supported at present',
                           priority='WARN', origin='map.predictmodel');
                     };
...
#   
   ...
   return ref _define_map(msfile);
} 

#
# Define demonstration function: return T if successful otherwise fail
#
const mapdemo:=function() {
   mymap:=map();
   note(paste("Demonstation of ", mymap.objectName()));
...
#
const maptest:=function() { fail "Not yet implemented";}

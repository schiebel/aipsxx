...
//#
//# $Id: imagerFactory.cc,v 19.6 2005/11/07 21:17:04 wyoung Exp $

#include <../imager/imagerFactory.h>
...
MethodResult imagerFactory::make(ApplicationObject *&newObject,
			      ...
			      Bool runConstructor)
{
  ...
  if (whichConstructor == "imager") {
    Parameter<String> thems(inputRecord, "thems", ParameterSet::In);
    Parameter<Bool> compress(inputRecord, "compress", ParameterSet::In); 
    if (runConstructor) {
      if(thems()==""){
	newObject = new imager();
      }
      else{
      MeasurementSet thisms(thems(), TableLock(TableLock::UserLocking),
			    Table::Update);
      newObject = new imager(thisms, compress());
      }
    }
  } else {

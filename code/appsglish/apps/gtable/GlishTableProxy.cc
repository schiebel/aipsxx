...
//#
//# $Id: GlishTableProxy.cc,v 19.17 2006/11/07 22:23:44 gvandiep Exp $
                                 ...
                                 String& message)
{
  ...
  GlishArray(gtab.get("columnNames")).get(columnNames);
  GlishArray(gtab.get("dataTypes")).get(dataTypes);
				 ...
				 String& message)
{
  ...
  tables_p[id].proxy().putColumn (column, row, nrow, incr, vh);
  eventStream.postEvent (returnEventName(event), GlishArray(True));
  return True;
}
				    ...
				    String& message)
{
  ...
  Record rec;
  value.toRecord (rec);
  tables_p[id].proxy().putVarColumn (column, row, nrow, incr, rec);
  eventStream.postEvent (returnEventName(event), GlishArray(True));
  return True;
}
				      ...
				      String& message)
{
  ...
  GlishValue value = location.get("value");
  ValueHolder vh = valueFromGlish(value);
  tables_p[id].proxy().putColumnSlice (column, row, nrow, incr,
				       blc, trc, inc, vh);
  eventStream.postEvent (returnEventName(event), GlishArray(True));
  return True;
}
				    ...
				    String& message)
{
  ...
  GlishValue value = location.get("value");
  ValueHolder vh = valueFromGlish(value);
  tables_p[id].proxy().putCellSlice (column, rownr, blc, trc, inc, vh);
  eventStream.postEvent (returnEventName(event), GlishArray(True));
  return True;
}

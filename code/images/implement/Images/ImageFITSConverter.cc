...
//#
//#
//# $Id: ImageFITSConverter.cc,v 19.7 2005/07/07 20:36:33 ddebonis Exp $
...
template<class HDUType>
void ImageFITSConverterImpl<HDUType>::FITSToImageOld(ImageInterface<Float>*& pNewImage,
                                                  String &error,
                                                  const String &imageName,
                                                  HDUType &fitsImage,
                                                  uInt memoryInMB,
                                                  Bool zeroBlanks)
{
    LogIO os(LogOrigin("ImageFITSConverterImpl", "FITSToImageOld", WHERE));
...
// Get Coordinate System
    ...
    Int stokesFITSValue = 1;
    CoordinateSystem coords =
       ImageFITSConverter::getCoordinateSystemOld(stokesFITSValue, header, os, shape, dropStokes);
    ndim = shape.nelements();
...
// Brightness Unit
    ...
    Unit bu = ImageFITSConverter::getBrightnessUnitOld(header, os);
    pNewImage->setUnits(bu);
...
// ImageInfo
    ...
    ImageInfo imageInfo = ImageFITSConverter::getImageInfoOld(header);

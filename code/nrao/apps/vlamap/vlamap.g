# vlamap: synthesis integration for the VLA one level higher than map.g
...
#
# $Id: vlamap.g,v 19.0 2003/07/16 03:44:23 aips2adm Exp $
...
include 'quanta.g';
#
# Define a vlamap instance
#
const _define_vlamap := function(msfile) {
#
   private := [=]
   public := [=]
#
                  ...
                  # CATALOG
                     ...
                     if (catalogname != 'Perley-Taylor 95') {
                        note('Only Perley-Tayor 95 supported at present',
                           priority='WARN', origin='vlamap.predictmodel');
                     };
...
#------------------------------------------------------------------------
#
   const public.makevlamap := function(selection=unset, calibration=unset,
                                       ftmachine=unset, imagingfields=unset,
                                       complist='', weighting=unset, 
                                       deconvolver=unset, restoringbeam=unset)
   {
   # Solve for calibration and make a VLA image
   #
...
#   
   ...
   return ref _define_vlamap(msfile);
} 

#
# Define demonstration function: return T if successful otherwise fail
#
const vlamapdemo:=function() {
   myvlamap:=vlamap();
   note(paste("Demonstation of ", myvlamap.objectName()));
...
#
const vlamaptest:=function() { fail "Not yet implemented";}

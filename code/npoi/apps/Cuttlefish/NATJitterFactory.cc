...
Description:
------------
This file contains NATJitterFactory::make( ) public member function.

Modification history:
---------------------
2001 Mar 20 - Nicholas Elias, USNO/NPOI
              File created with public member function
	      NATJitterFactory::make( ).
...
#include "NATJitterFactory.h" // NATJitter factory
...
MethodResult NATJitterFactory::make( ApplicationObject*& poObject,
  ...
  if ( oWhichConstructor.matches( "NATJITTER" ) ) {
  
    // NATJitter{ } with the standard constructor
    ...
    if ( bMakeObject ) {
      try {
        poObject = new NATJitter( file(), (uInt) inputbeam() );
      }
        ...
        oReturnValue = GeneralStatus::ermsg(
            "NATJitter{ } error\n" + oAipsError.getMesg(), "NATJitterFactory",
            "make" ).getMesg();
      }
    }

  } else if ( oWhichConstructor.matches( "NATJITTER_AVERAGE" ) ) {

    // NATJitter{ } with the average constructor
    ...
    if ( bMakeObject ) {
      try {
        poObject = new NATJitter( objectid(), x(), xmin(), xmax(), token(),
  ...
  } else if ( oWhichConstructor.matches( "NATJITTER_CLONE" ) ) {

    // NATJitter{ } with the clone constructor
    ...
    if ( bMakeObject ) {
      try {
        poObject = new NATJitter( objectid(), xmin(), xmax(), token(), keep() );
      }
      catch ( AipsError oAipsError ) {
        if ( poObject != NULL ) {
          delete poObject;
  ...
  } else if ( oWhichConstructor.matches( "NATJITTER_INTERPOLATE" ) ) {

    // NATJitter{ } with the interpolate constructor
    ...
    if ( bMakeObject ) {
      try {
        poObject = new NATJitter( objectid(), x(), token(), keep(), interp(),
            xminbox(), xmaxbox() );
      }
      catch ( AipsError oAipsError ) {
        if ( poObject != NULL ) {
    ...
    oReturnValue = GeneralStatus::ermsg(
        "Cannot determine which class/constructor to use ", "NATJitterFactory",
        "make" ).getMesg();
  ...
  if ( oReturnValue.ok() && bMakeObject && poObject == NULL ) {
    oReturnValue = GeneralStatus::ermsg(
        "Insufficient memory to create a new NATJitter object", "NATJitterFactory",
        "make" ).getMesg();
  }

# __obdata1.g is part of the Cuttlefish server
...
#
# $Id: __obdata1.g,v 19.0 2003/07/16 06:02:47 aips2adm Exp $
# ------------------------------------------------------------------------------
...
# Description:
# ------------
# This file contains the glish functions for creating 1D output-beam tools.
# These functions are not typically called directly by the user.

# glish functions:
# ----------------
# __obdata1_define_members, __obdata1, __obdata1_average, __obdata1_clone,
# __obdata1_interpolate.

# Modification history:
# ---------------------
# 2001 May 11 - Nicholas Elias, USNO/NPOI
#               File created with glish functions __obdata1_define_members( ),
#               __obdata1( ), __obdata1_average( ), obdata1_clone( ), and
#               __obdata1_interpolate( ).
...
if ( !include 'whenever_manager.g' ) {
  throw( 'Cannot include whenever_manager.g ...', origin = '__obdata1' );
}

if ( !include 'gdc1token.g' ) {
  throw( 'Cannot include gdc1token.g ...', origin = '__obdata1' );
}

if ( !include 'hds.g' ) {
  throw( 'Cannot include hds.g ...', origin = '__obdata1' );
}

if ( !include 'obconfig.g' ) {
  throw( 'Cannot include obconfig.g ...', origin = '__obdata1' );
}

if ( !include 'scaninfo.g' ) {
  throw( 'Cannot include scaninfo.g ...', origin = '__obdata1' );
}

if ( !include '__obdata1_guimore.g' ) {
  throw( 'Cannot include __obdata1_guimore.g ...', origin = '__obdata1' );
}

if ( !include '__obdata1_public.g' ) {
  throw( 'Cannot include __obdata1_public.g ...', origin = '__obdata1' );
}

# ------------------------------------------------------------------------------

# __define_obdata1_members

# Description:
# ------------
# This glish function defines the member functions for a 1D output-beam tool.
...
const __define_obdata1_members := function( ref agent, id, constructor,
  ...
  __obdata1_public( gui, private, public );

  __obdata1_public( gui, private, public );
  __obdata1_guimore( gui, w, private, public );
  ...
  val private.window.default := spaste( annotation, public.filetail(),
      annotation, ' ', to_lower( public.object() ), ' ', public.outbeam(),
      ' ', public.baseline() );
...
# Description
# -----------
# This glish function creates a 1D output-beam tool.  This function is typically
# not called directly by the user.

# Inputs:
# -------
# file            - The file name.
# outbeam         - The output-beam number.
...
# Outputs:
# --------
# The 1D output-beam tool, returned via the glish function value.
...
const __obdata1 := function( file, outbeam, baseline, constructor,
  ...
  if ( !is_integer( outbeam ) ) {
    return( throw( 'Invalid output-beam number ...', origin = gconstructor ) );
  }
  
  if ( !is_integer( baseline ) ) {
    return( throw( 'Invalid baseline number ...', origin = gconstructor ) );
  }
  ...
  args := [file = file, outbeam = outbeam, baseline = baseline];
  ...
  # Return the 1D output-beam tool

  gui := F;
  w := F;
  private := F;
  
  public := __define_obdata1_members( agent, id, constructor, gconstructor,
      function2init, averagefunc, clonefunc, interpolatefunc, host,
      forcenewserver, gui, w, private );
...
# Description:
# ------------
# This glish function creates an averaged 1D output-beam tool.  Typically, this
...
# Outputs:
# --------
# The averaged 1D output-beam tool, returned via the glish function value.
...
const __obdata1_average := function( objectid, constructor, function2init,
  ...
  # Return the averaged 1D output-beam tool
...
# Description:
# ------------
# This glish function creates a cloned 1D output-beam tool.  Typically, this is
# not called directly by the user.
...
# Outputs:
# --------
# The cloned 1D output-beam tool, returned via the glish function value.
...
const __obdata1_clone := function( objectid, constructor, function2init,
  ...
  # Return the cloned 1D output-beam tool
...
# Description:
# ------------
# This glish function creates an interpolated 1D output-beam tool.  Typically,
...
# Outputs:
# --------
# The interpolated 1D output-beam tool, returned via the glish function value.
...
const __obdata1_interpolate := function( objectid, constructor, function2init,
  ...
  # Return the interpolated 1D output-beam tool

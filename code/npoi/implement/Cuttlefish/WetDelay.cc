//#WetDelay.cc is part of the Cuttlefish server
...
//#
//# $Id: WetDelay.cc,v 19.1 2004/08/25 05:49:26 gvandiep Exp $
// -----------------------------------------------------------------------------
...
Description:
------------
This file contains the WetDelay{ } class member functions.

Public member functions:
------------------------
WetDelay (5 versions), ~WetDelay, baseTool, dumpHDS, saveHDS, tool, version.
...
2001 May 10 - Nicholas Elias, USNO/NPOI
              File created with public member functions WetDelay( ) (standard,
              average, clone, interpolate, copy versions), ~WetDelay( ),
...
#include <npoi/Cuttlefish/WetDelay.h> // WetDelay class
...
WetDelay::WetDelay (standard)

Description:
------------
This public member function constructs a WetDelay{ } object.
...
WetDelay::WetDelay( String& oFileIn, const uInt& uiInputBeamIn )
    : __IBData1() {
  
  // Initialize the __IBData1{ } base class
  
  String oObject = String( "WETDELAY" );
  ...
  sprintf( acYLabel, "Wet Delay #%u (m)", uiInputBeamIn );
...
WetDelay::WetDelay( const ObjectID& oObjectIDIn, const Vector<Double>& oXIn,
...
WetDelay::WetDelay( const ObjectID& oObjectIDIn, Double& dXMinIn, Double& dXMaxIn,
...
Inputs:
-------
oWetDelayIn - The WetDelay{ } object.
...
WetDelay::WetDelay( const WetDelay& oWetDelayIn )
    : __IBData1( (const __IBData1&) oWetDelayIn ) {
...
Description:
------------
This public member function destructs a WetDelay{ } object.
...
Description:
------------
This public member function dumps the wet-delay data into an HDS file (not the
present one).
...
void WetDelay::dumpHDS( String& oFileIn, Double& dXMinIn, Double& dXMaxIn,
  ...
  if ( oFileIn.matches( oFile ) ) {
    throw( ermsg( "Cannot use dumpHDS( ) with the present file", "WetDelay",
        "dumpHDS" ) );
  }

  if ( !checkToken( oTokenIn ) ) {
    throw( ermsg( "Invalid token argument(s)", "WetDelay", "dumpHDS" ) );
  }

  if ( !checkX( dXMinIn, dXMaxIn ) ) {
    throw( ermsg( "Invalid x argument(s)", "WetDelay", "dumpHDS" ) );
  }
  ...
  // Dump the wet-delay errors (negative values are flagged)
...
Description:
------------
This public member function save the wet-delay data into the present HDS file.
...
void WetDelay::saveHDS( void ) const {
...
Description:
------------
This public member function returns the WetDelay{ } version.
...
Outputs:
--------
The WetDelay{ } version, returned via the function value.
...
String WetDelay::version( void ) const {
...
String WetDelay::baseTool( void ) const {
...
String WetDelay::tool( void ) const {
...
String WetDelay::className( void ) const {
...
Vector<String> WetDelay::methods( void ) const {
...
Vector<String> WetDelay::noTraceMethods( void ) const {
...
MethodResult WetDelay::runMethod( uInt uiMethod, ParameterSet &oParameters,
    Bool bRunMethod ) {
...
Description:
------------
This protected member function initializes a WetDelay{ } object.
...
Description:
------------
This private member function loads the wet-delay data from an HDS file.
...
void WetDelay::loadHDS( String& oFileIn, const uInt& uiInputBeamIn,
  ...
  try {
    ...
    throw( ermsg( "Cannot load wet delays\n" + oAipsError.getMesg(),
        "WetDelay", "loadHDS" ) );
  }
  
  
  // Load the wet-delay errors
  ...
  try {
    ...
    throw( ermsg( "Cannot load wet-delay errors\n" + oAipsError.getMesg(),
        "WetDelay", "loadHDS" ) );
  }
...
void WetDelay::initMethods( void ) {

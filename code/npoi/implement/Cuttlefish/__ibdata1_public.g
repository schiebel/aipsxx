# __ibdata1_public.g is part of the Cuttlefish server
# Copyright (C) 2001
...
#
# $Id: __ibdata1_public.g,v 19.0 2003/07/16 06:02:39 aips2adm Exp $
# ------------------------------------------------------------------------------
...
#               File created with glish function __ibdata1_public( ).
...
# Description
# -----------
# This glish function adds public member functions to the 1D input-beam tool.
...
const __ibdata1_public := function( ref gui, ref private, ref public ) {
  ...
  const val public.changex := function( xtoken ) {
    ...
    if ( !is_fail( ret ) ) {
      __ibdata1_guimore_xaxis_update( xtoken, gui, private, public );
      private.plot();
      return( ret );
    } else {
      member := spaste( private.gconstructor, '.changex' );
  ...
  # Define the 'inputbeam' public and private member functions
  
  const val public.inputbeam := function( ) {
    wider private;
    return( private.inputbeam() );
  }

  val private.inputBeamRec :=
      [_method = 'inputBeam', _sequence = private.id._sequence];
  
  const val private.inputbeam := function( ) {
    wider private;
    return( defaultservers.run( private.agent, private.inputBeamRec ) );
  }
  ...
  # Define the 'ibconfig' public member function

  const val public.ibconfig := function( ) {
    wider public;
    file := public.file();
    host := public.host();
    forcenewserver := public.forcenewserver();
    return( ibconfig( file, host, forcenewserver ) );
  }

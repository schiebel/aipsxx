//#GDC1.h is part of the GDC server
...
//#
//# $Id: GDC1.h,v 19.4 2004/11/30 17:50:40 ddebonis Exp $
// -----------------------------------------------------------------------------
...
Description:
------------
This file contains the include information for the GDC1.cc file.
...
// <summary>A class for manipulating 1-dimensional data</summary>
...
// <synopsis>
// GDC1{ } is a class designed for the manipulation of 1-dimensional data (x,
// y, xerr, yerr, flag).  Manipulation means any of the following: plotting,
// flagging, interpolation, statistics.
// </synopsis>
...
class GDC1 : public GeneralStatus, public ApplicationObject {

  public:

    // Create a GDC1{ } object.
    GDC1( void );
    GDC1( const Vector<Double>& oXIn, const Vector<Double>& oYIn,
        const Vector<Double>& oXErrIn, const Vector<Double>& oYErrIn,
        const Vector<Bool>& oFlagIn, const Bool& bHMSIn = False );
    GDC1( String& oFileIn, const Bool& bHMSIn = False );
    GDC1( const ObjectID& oObjectIDIn, Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn );
    GDC1( const ObjectID& oObjectIDIn, const Vector<Double>& oXIn,
    ...
    // Clone the object.
    GDC1* clone( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn ) const;

    // Average the object.
    GDC1* average( const Vector<Double>& oXIn, Double& dXMinIn,
        Double& dXMaxIn, const Bool& bKeepIn, const Bool& bWeightIn,
        const Bool& bXCalcIn, String& oInterpIn );

    // Interpolate the object.
    GDC1* interpolate( const Vector<Double>& oXIn, const Bool& bKeepIn,
    ...
    // Return the length.
    uInt length( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn ) const;
    ...
    // Return the y values.
    Vector<Double> y( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
    ...
    // Return the y errors.
    Vector<Double> yErr( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bOrigIn ) const;
    ...
    Vector<Bool> interp( Double& dXMinIn, Double& dXMaxIn ) const;
    ...
    Double xMax( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bPlotIn ) const;

    // Return the minimum x value.
    Double xMin( const Bool& bPlotIn = False ) const;
    Double xMin( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bPlotIn ) const;

    // Return the maximum y value.
    Double yMax( const Bool& bPlotIn = False ) const;
    Double yMax( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bPlotIn ) const;

    // Return the minimum y value.
    Double yMin( const Bool& bPlotIn = False ) const;
    Double yMin( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bPlotIn ) const;

    // Return the maximum x error.
    Double xErrMax( void ) const;
    Double xErrMax( Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn ) const;

    // Return the minimum x error.
    Double xErrMin( void ) const;
    Double xErrMin( Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn ) const;

    // Return the maximum y error.
    Double yErrMax( void ) const;
    Double yErrMax( Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn ) const;

    // Return the minimum y error.
    Double yErrMin( void ) const;
    Double yErrMin( Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn ) const;
    
    // Return the flagged data indices.
    Vector<Int> flagged( Double& dXMinIn, Double& dXMaxIn ) const;

    // Return the interpolated data indices.
    Vector<Int> interpolated( Double& dXMinIn, Double& dXMaxIn ) const;

    // Return the y mean value.
    Double mean( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bWeightIn ) const;

    // Return the y mean error.
    Double meanErr( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bWeightIn ) const;

    // Return the y standard deviation.
    Double stdDev( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bWeightIn ) const;

    // Return the y variance.
    Double variance( Double& dXMinIn, Double& dXMaxIn, const Bool& bKeepIn,
        const Bool& bWeightIn ) const;
    
    // Flag the data according to the x values.
    void setFlagX( Double& dXMinIn, Double& dXMaxIn,
    ...
    void interpolateXY( Double& dXMinIn, Double& dXMaxIn,
        const Bool& bKeepIn, String& oInterpIn, Double& dYMinIn,
        Double& dYMaxIn, Double& dXMinBoxIn, Double& dXMaxBoxIn,
        Double& dYMinBoxIn, Double& dYMaxBoxIn );
    ...
    // Return the glish tool name (must be "gdc1").
    ...
    // Return the list of GDC1{ } methods.
    virtual Vector<String> methods( void ) const;

    // Return the list of GDC1{ } methods where no trace is printed.
    virtual Vector<String> noTraceMethods( void ) const;

    // The GDC server uses this method to pass arguments to GDC1{ } methods and
    // run them.
  ...
  protected:
    ...
    void initialize( const Vector<Double>& oXIn, const Vector<Double>& oYIn,
        const Vector<Double>& oXErrIn, const Vector<Double>& oYErrIn,
  ...
  private:
    ...
    void loadASCII( String& oFileIn, Vector<Double>& oXOut,
        ...
        Vector<Double>& oYErrOut, Vector<Bool>& oFlagOut );

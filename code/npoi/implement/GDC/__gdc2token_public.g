# __gdc2token_public.g is part of the GDC server
...
#
# $Id: __gdc2token_public.g,v 19.0 2003/07/16 06:03:39 aips2adm Exp $
# ------------------------------------------------------------------------------

# __gdc2token_public.g
...
# private) member functions for gdc2token tools.  NB: These functions should be
# called only by gdc2token tools.
...
#               File created with glish function __gdc2token_public( ).
...
const __gdc2token_public := function( ref gui, ref w, ref private,
    ref public ) { 
  
  # Define the 'tool' public and private member functions

  if ( spaste( split( private.gconstructor, '' )[1:9] ) == 'gdc2token' ) {
  ...
  if ( private.gconstructor == 'gdc2token_ascii' ) {
...
#   return( __gdc1token_maingui( window, gui, w, private, public ) );
  ...
  const val public.dumpascii := function( file, xmin = [], xmax = [],
    ...
    if ( !private.checkcolumn( column ) ) {
      return( throw( 'Invalid column(s) ...', origin = member ) );
    }
    if ( !is_boolean( keep ) ) {
      return( throw( 'Invalid \'keep-flags\' boolean ...', origin = member ) );
    }
    return( private.dumpascii( file, xmin, xmax, token, column, keep ) );
  }

  val private.dumpasciiRec :=
      [_method = 'dumpASCII', _sequence = private.id._sequence];
  
  const val private.dumpascii := function( file, xmin, xmax, token, column,
      keep ) {
    wider private;
    ...
    val private.dumpasciiRec.columnarg := column;
  ...
  if ( spaste( split( private.gconstructor, '' )[1:9] ) == 'gdc2token' ) {
    ...
    const val public.clone := function( xmin = [], xmax = [], token = "",
      ...
      if ( !private.checkcolumn( column ) ) {
        return( throw( 'Invalid column ID(s) ...', origin = member ) );
      }
      if ( !is_boolean( keep ) ) {
        return( throw( 'Invalid \'keep-flags\' boolean ...',
            origin = member ) );
      }
      return( private.clonefunc( public.id(), xmin, xmax, token, column, keep,
...
#  }
  ...
  const val public.length := function( xmin = [], xmax = [], token = "",
    ...
    return( private.length( xmin, xmax, token, column, keep ) );
  }

  val private.lengthRec := [_method = 'length',
      _sequence = private.id._sequence];
  
  const val private.length := function( xmin, xmax, token, column, keep ) {
    wider private;
    val private.lengthRec.xmin := xmin;
    val private.lengthRec.xmax := xmax;
    val private.lengthRec.tokenarg := token;
    val private.lengthRec.columnarg := column;
  ...
  const val public.x := function( xmin = [], xmax = [], token = "",
      column = [], keep = F ) {
    wider private;
  ...
  const val public.y := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, orig = F ) {
    wider private;
    ...
    return( private.y( xmin, xmax, token, column, keep, orig ) );
  }

  val private.yRec := [_method = 'y', _sequence = private.id._sequence];
  
  const val private.y := function( xmin, xmax, token, column, keep, orig ) {
    wider private;
    val private.yRec.xmin := xmin;
    val private.yRec.xmax := xmax;
    val private.yRec.tokenarg := token;
    val private.yRec.columnarg := column;
  ...
  # Define the 'columntype' public and private member functions

  val private.columnTypeRec :=
      [_method = 'columnType', _sequence = private.id._sequence];
  
  const val public.columntype := function( ) {
    wider private;
    return( private.columntype() );
  }
  
  const val private.columntype := function( ) {
    wider private;
    return( defaultservers.run( private.agent, private.columnTypeRec ) );
  }
  
  
  # Define the 'columnlist' public and private member functions

  val private.columnListRec :=
      [_method = 'columnList', _sequence = private.id._sequence];
  
  const val public.columnlist := function( ) {
    wider private;
    return( private.columnlist() );
  }
  
  const val private.columnlist := function( ) {
    wider private;
    return( defaultservers.run( private.agent, private.columnListRec ) );
  }
  ...
  const val public.xmin := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, plot = F ) {
    wider private;
  ...
  const val public.xmax := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, plot = F ) {
    wider private;
  ...
  const val public.xerrmin := function( xmin = [], xmax = [], token = "",
    ...
    return( private.xerrmin( xmin, xmax, token, column, keep ) );
  }

  val private.xErrMinRec := [_method = 'xErrMin',
      _sequence = private.id._sequence];
  
  const val private.xerrmin := function( xmin, xmax, token, column, keep ) {
    wider private;
    val private.xErrMinRec.xmin := xmin;
    val private.xErrMinRec.xmax := xmax;
    val private.xErrMinRec.tokenarg := token;
    val private.xErrMinRec.columnarg := column;
  ...
  const val public.xerrmax := function( xmin = [], xmax = [], token = "",
    ...
    return( private.xerrmax( xmin, xmax, token, column, keep ) );
  }

  val private.xErrMaxRec := [_method = 'xErrMax',
      _sequence = private.id._sequence];
  
  const val private.xerrmax := function( xmin, xmax, token, column, keep ) {
    wider private;
    val private.xErrMaxRec.xmin := xmin;
    val private.xErrMaxRec.xmax := xmax;
    val private.xErrMaxRec.tokenarg := token;
    val private.xErrMaxRec.columnarg := column;
  ...
  const val public.ymin := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, plot = F ) {
    wider private;
  ...
  const val public.ymax := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, plot = F ) {
    wider private;
  ...
  const val public.yerrmin := function( xmin = [], xmax = [], token = "",
    ...
    return( private.yerrmin( xmin, xmax, token, column, keep ) );
  }

  val private.yErrMinRec := [_method = 'yErrMin',
      _sequence = private.id._sequence];
  
  const val private.yerrmin := function( xmin, xmax, token, column, keep ) {
    wider private;
    val private.yErrMinRec.xmin := xmin;
    val private.yErrMinRec.xmax := xmax;
    val private.yErrMinRec.tokenarg := token;
    val private.yErrMinRec.columnarg := column;
  ...
  const val public.yerrmax := function( xmin = [], xmax = [], token = "",
    ...
    return( private.yerrmax( xmin, xmax, token, column, keep ) );
  }

  val private.yErrMaxRec := [_method = 'yErrMax',
      _sequence = private.id._sequence];
  
  const val private.yerrmax := function( xmin, xmax, token, column, keep ) {
    wider private;
    val private.yErrMaxRec.xmin := xmin;
    val private.yErrMaxRec.xmax := xmax;
    val private.yErrMaxRec.tokenarg := token;
    val private.yErrMaxRec.columnarg := column;
  ...
  const val public.flagged := function( xmin = [], xmax = [], token = "",
    ...
    if ( !private.checkcolumn( column ) ) {
      return( throw( 'Invalid column ...', origin = member ) );
    }
    return( private.flagged( xmin, xmax, token, column ) );
  }

  val private.flaggedRec := [_method = 'flagged',
      _sequence = private.id._sequence];
  
  const val private.flagged := function( xmin, xmax, token, column ) {
    wider private;
    val private.flaggedRec.xmin := xmin;
    val private.flaggedRec.xmax := xmax;
    val private.flaggedRec.tokenarg := token;
    val private.flaggedRec.columnarg := column;
  ...
  const val public.mean := function( xmin = [], xmax = [], token = "",
      column = [], keep = F, weight = F ) {
    wider private;
  ...
  const val public.meanerr := function( xmin = [], xmax = [], token = "",
    ...
    return( private.meanerr( xmin, xmax, token, column, keep, weight ) );
  }

  val private.meanErrRec := [_method = 'meanErr',
      _sequence = private.id._sequence];
  
  const val private.meanerr := function( xmin, xmax, token, column, keep,
      weight ) {
    wider private;
    val private.meanErrRec.xmin := xmin;
    val private.meanErrRec.xmax := xmax;
    val private.meanErrRec.tokenarg := token;
    val private.meanErrRec.columnarg := column;
  ...
  const val public.stddev := function( xmin = [], xmax = [], token = "",
    ...
    return( private.stddev( xmin, xmax, token, column, keep, weight ) );
  }

  val private.stdDevRec := [_method = 'stdDev',
      _sequence = private.id._sequence];
  
  const val private.stddev := function( xmin, xmax, token, column, keep,
      weight ) {
    wider private;
    val private.stdDevRec.xmin := xmin;
    val private.stdDevRec.xmax := xmax;
    val private.stdDevRec.tokenarg := token;
    val private.stdDevRec.columnarg := column;
  ...
  const val public.setflagx := function( xmin = [], xmax = [], token = "",
    ...
    return( private.setflagx( xmin, xmax, token, column, flag ) );
  }

  val private.setFlagXRec :=
      [_method = 'setFlagX', _sequence = private.id._sequence];
  
  const val private.setflagx := function( xmin, xmax, token, column, flag ) {
    ...
    val private.setFlagXRec.columnarg := column;
...
#    private.plot();
  ...
  const val public.setflagxy := function( xmin = [], xmax = [], ymin = [],
    ...
    return( private.setflagxy( xmin, xmax, ymin, ymax, token, column, flag ) );
  }
  ...
  const val private.setflagxy := function( xmin, xmax, ymin, ymax, token,
    ...
    val private.setFlagXYRec.columnarg := column;

# __gdc2token_private.g is part of the GDC server
...
#
# $Id: __gdc2token_private.g,v 19.0 2003/07/16 06:03:38 aips2adm Exp $
# ------------------------------------------------------------------------------

# __gdc2token_private.g
...
# for gdc2token tools.  NB: These functions should be called only by gdc2token
# tools.
...
#               File created with glish function __gdc2token_private( ).
...
# Description:
# ------------
# This glish function creates private member functions for a gdc2token tool.
...
const __gdc2token_private := function( ref gui, ref private, ref public ) { 
  
  # Define the 'checkgdc2token' private member function
  
  const val private.checkgdc2token := function( tool ) {
    if ( !is_function( tool ) ) {
      return( F );
    }
  ...
  # Define the 'checkcolumn' private member function

  val private.checkColumnRec :=
      [_method = 'checkColumn', _sequence = private.id._sequence];
  
  const val private.checkcolumn := function( ref column ) {
    wider private;
    if ( !private.getargcheck() ) {
      return( T );
    }
    val private.checkColumnRec.columnarg := as_string( column );
    columnList := defaultservers.run( private.agent, private.checkColumnRec );
    check := as_boolean( columnList[1] );
    if ( check ) {
      val column := columnList[2:length(columnList)];
    }
    return( check );
  }

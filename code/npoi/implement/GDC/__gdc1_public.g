# __gdc1_public.g is part of the GDC server
...
#
# $Id: __gdc1_public.g,v 19.0 2003/07/16 06:03:34 aips2adm Exp $
# ------------------------------------------------------------------------------
...
# private) member functions for gdc1 tools.  NB: These functions should be
# called only by gdc1 tools.
...
#               File created with glish function __gdc1_public( ).
...
const __gdc1_public := function( ref gui, ref w, ref private, ref public ) { 
  
  # Define the 'tool' public and private member functions

  if ( spaste( split( private.gconstructor, '' )[1:4] ) == 'gdc1' ) {
  ...
  if ( private.gconstructor == 'gdc1_ascii' ) {
  ...
  const val public.maingui := function( window = '' ) {
    ...
    return( __gdc1_maingui( window, gui, w, private, public ) );
  }
  ...
  const val public.dumpascii := function( file, xmin = [], xmax = [],
    ...
    return( private.dumpascii( file, xmin, xmax, keep ) );
  }

  val private.dumpasciiRec :=
      [_method = 'dumpASCII', _sequence = private.id._sequence];
  
  const val private.dumpascii := function( file, xmin, xmax, keep ) {
    wider private;
  ...
  if ( spaste( split( private.gconstructor, '' )[1:4] ) == 'gdc1' ) {
    ...
    const val public.clone := function( xmin = [], xmax = [], keep = F ) {
      ...
      return( private.clonefunc( public.id(), xmin, xmax, keep, public.host(),
          public.forcenewserver() ) );
    }
  ...
  if ( spaste( split( private.gconstructor, '' )[1:4] ) == 'gdc1' ) {
    ...
    const val public.average := function( x, xmin = [], xmax = [], keep = F,
        weight = F, xcalc = F, interp = 'SPLINE' ) {
      wider private, public;
      ...
      return( private.averagefunc( public.id(), x, xmin, xmax, keep, weight,
          xcalc, interp, public.host(), public.forcenewserver() ) );
    }
  ...
  if ( spaste( split( private.gconstructor, '' )[1:4] ) == 'gdc1' ) {
    ...
    const val public.interpolate := function( x, keep = F, interp = 'SPLINE',
        xminbox = [], xmaxbox = [] ) {
      wider private, public;
      ...
      return( private.interpolatefunc( public.id(), x, keep, interp, xminbox,
          xmaxbox, public.host(), public.forcenewserver() ) );
    }
  ...
  const val public.yinterpolate := function( x, keep = F, interp = 'SPLINE',
      xminbox = [], xmaxbox = [] ) {
    wider private, public;
    ...
    return( private.yinterpolate( x, keep, interp, xminbox, xmaxbox ) );
  }

  val private.yinterpolateRec := [_method = 'yInterpolate',
      _sequence = private.id._sequence];
  
  const val private.yinterpolate := function( x, keep, interp, xminbox,
      xmaxbox ) {
    wider private;
  ...
  const val public.calc := function( myfunc, ..., xmin = [], xmax = [],
    ...
    xTemp := private.x( xmin, xmax, keep );
    if ( length( xTemp ) < 1 ) {
      return( throw( 'No data in range ...', origin = member ) );
    }
    xerrTemp := private.xerr( xmin, xmax, keep );
    yTemp := private.y( xmin, xmax, keep );
    yerrTemp := private.yerr( xmin, xmax, keep );
    retval := [=];
    retval.x := as_double([]);
    retval.y := as_double([]);
    ...
    if ( is_fail( myfunc( xTemp, yTemp, xerrTemp, yerrTemp, retval.x,
        retval.y, retval.xerr, retval.yerr, ... ) ) ) {
      return( throw( 'Calculation error ...', origin = member ) );
    }
    gdc1Temp := gdc1_standard( retval.x, retval.y, retval.xerr, retval.yerr );
    return( ref gdc1Temp );
  }
  ...
  const val public.length := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.length( xmin, xmax, keep ) );
  }

  val private.lengthRec := [_method = 'length',
      _sequence = private.id._sequence];
  
  const val private.length := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.x := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.x( xmin, xmax, keep ) );
  }

  val private.xRec := [_method = 'x', _sequence = private.id._sequence];
  
  const val private.x := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.y := function( xmin = [], xmax = [], keep = F, orig = F ) {
    wider private;
    ...
    return( private.y( xmin, xmax, keep, orig ) );
  }

  val private.yRec := [_method = 'y', _sequence = private.id._sequence];
  
  const val private.y := function( xmin, xmax, keep, orig ) {
    wider private;
  ...
  const val public.xerr := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.xerr( xmin, xmax, keep ) );
  }

  val private.xErrRec := [_method = 'xErr', _sequence = private.id._sequence];
  
  const val private.xerr := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.yerr := function( xmin = [], xmax = [], keep = F,
      orig = F ) {
    wider private;
    ...
    return( private.yerr( xmin, xmax, keep, orig ) );
  }

  val private.yErrRec := [_method = 'yErr', _sequence = private.id._sequence];
  
  const val private.yerr := function( xmin, xmax, keep, orig ) {
    wider private;
  ...
  const val public.flag := function( xmin = [], xmax = [], orig = F ) {
    wider private;
    ...
    return( private.flag( xmin, xmax, orig ) );
  }

  val private.flagRec := [_method = 'flag', _sequence = private.id._sequence];
  
  const val private.flag := function( xmin, xmax, orig ) {
    wider private;
  ...
  const val public.interp := function( xmin = [], xmax = [] ) {
    wider private;
    member := spaste( private.gconstructor, '.interp' );
    if ( !private.checkx( xmin, xmax ) ) {
      return( throw( 'Invalid x argument(s) ...', origin = member ) );
    }
    return( private.interp( xmin, xmax ) );
  }

  val private.interpRec :=
      [_method = 'interp', _sequence = private.id._sequence];
  
  const val private.interp := function( xmin, xmax ) {
    wider private;
  ...
  const val public.index := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.index( xmin, xmax, keep ) );
  }

  val private.indexRec := [_method = 'index', _sequence = private.id._sequence];
  
  const val private.index := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.xmin := function( xmin = [], xmax = [], keep = F,
      plot = F ) {
    wider private;
    ...
    return( private.xmin( xmin, xmax, keep, plot ) );
  }

  val private.xminRec := [_method = 'xMin', _sequence = private.id._sequence];
  
  const val private.xmin := function( xmin, xmax, keep, plot ) {
    wider private;
  ...
  const val public.xmax := function( xmin = [], xmax = [], keep = F,
      plot = F ) {
    wider private;
    ...
    return( private.xmax( xmin, xmax, keep, plot ) );
  }

  val private.xmaxRec := [_method = 'xMax', _sequence = private.id._sequence];
  
  const val private.xmax := function( xmin, xmax, keep, plot ) {
    wider private;
  ...
  const val public.xerrmin := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.xerrmin( xmin, xmax, keep ) );
  }

  val private.xErrMinRec := [_method = 'xErrMin',
      _sequence = private.id._sequence];
  
  const val private.xerrmin := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.xerrmax := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.xerrmax( xmin, xmax, keep ) );
  }

  val private.xErrMaxRec := [_method = 'xErrMax',
      _sequence = private.id._sequence];
  
  const val private.xerrmax := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.ymin := function( xmin = [], xmax = [], keep = F,
      plot = F ) {
    wider private;
    ...
    return( private.ymin( xmin, xmax, keep, plot ) );
  }

  val private.yminRec := [_method = 'yMin', _sequence = private.id._sequence];
  
  const val private.ymin := function( xmin, xmax, keep, plot ) {
    wider private;
  ...
  const val public.ymax := function( xmin = [], xmax = [], keep = F,
      plot = F ) {
    wider private;
    ...
    return( private.ymax( xmin, xmax, keep, plot ) );
  }

  val private.ymaxRec := [_method = 'yMax', _sequence = private.id._sequence];
  
  const val private.ymax := function( xmin, xmax, keep, plot ) {
    wider private;
  ...
  const val public.yerrmin := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.yerrmin( xmin, xmax, keep ) );
  }

  val private.yErrMinRec := [_method = 'yErrMin',
      _sequence = private.id._sequence];
  
  const val private.yerrmin := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.yerrmax := function( xmin = [], xmax = [], keep = F ) {
    wider private;
    ...
    return( private.yerrmax( xmin, xmax, keep ) );
  }

  val private.yErrMaxRec := [_method = 'yErrMax',
      _sequence = private.id._sequence];
  
  const val private.yerrmax := function( xmin, xmax, keep ) {
    wider private;
  ...
  const val public.flagged := function( xmin = [], xmax = [] ) {
    wider private;
    member := spaste( private.gconstructor, '.flagged' );
    if ( !private.checkx( xmin, xmax ) ) {
      return( throw( 'Invalid x argument(s) ...', origin = member ) );
    }
    return( private.flagged( xmin, xmax ) );
  }

  val private.flaggedRec := [_method = 'flagged',
      _sequence = private.id._sequence];
  
  const val private.flagged := function( xmin, xmax ) {
    wider private;
  ...
  const val public.interpolated := function( xmin = [], xmax = [] ) {
    ...
    return( private.interpolated( xmin, xmax ) );
  }

  val private.interpolatedRec := [_method = 'interpolated',
      _sequence = private.id._sequence];
  
  const val private.interpolated := function( xmin, xmax ) {
    wider private;
  ...
  const val public.mean := function( xmin = [], xmax = [], keep = F,
      weight = F ) {
    wider private;
    ...
    return( private.mean( xmin, xmax, keep, weight ) );
  }

  val private.meanRec := [_method = 'mean', _sequence = private.id._sequence];
  
  const val private.mean := function( xmin, xmax, keep, weight ) {
    wider private;
  ...
  const val public.meanerr := function( xmin = [], xmax = [], keep = F,
      weight = F ) {
    wider private;
    ...
    return( private.meanerr( xmin, xmax, keep, weight ) );
  }

  val private.meanErrRec := [_method = 'meanErr',
      _sequence = private.id._sequence];
  
  const val private.meanerr := function( xmin, xmax, keep, weight ) {
    wider private;
  ...
  const val public.variance := function( xmin = [], xmax = [], keep = F,
      weight = F ) {
    wider private;
    ...
    return( private.variance( xmin, xmax, keep, weight ) );
  }

  val private.varianceRec :=
      [_method = 'variance', _sequence = private.id._sequence];
  
  const val private.variance := function( xmin, xmax, keep, weight ) {
    wider private;
  ...
  const val public.stddev := function( xmin = [], xmax = [], keep = F,
      weight = F ) {
    wider private;
    ...
    return( private.stddev( xmin, xmax, keep, weight ) );
  }

  val private.stdDevRec := [_method = 'stdDev',
      _sequence = private.id._sequence];
  
  const val private.stddev := function( xmin, xmax, keep, weight ) {
    wider private;
  ...
  const val public.setflagx := function( xmin = [], xmax = [], flag = T ) {
    wider private;
    ...
    return( private.setflagx( xmin, xmax, flag ) );
  }

  val private.setFlagXRec :=
      [_method = 'setFlagX', _sequence = private.id._sequence];
  
  const val private.setflagx := function( xmin, xmax, flag ) {
  ...
  const val public.setflagxy := function( xmin = [], xmax = [], ymin = [],
    ...
    return( private.setflagxy( xmin, xmax, ymin, ymax, flag ) );
  }

  val private.setFlagXYRec :=
      [_method = 'setFlagXY', _sequence = private.id._sequence];
  
  const val private.setflagxy := function( xmin, xmax, ymin, ymax, flag ) {
    wider gui, private, public;
    member := spaste( private.gconstructor, '.setflagxy' );
  ...
  const val public.interpolatex := function( xmin = [], xmax = [], keep = F,
    ...
    return( private.interpolatex( xmin, xmax, keep, interp, xminbox,
        xmaxbox ) );
  }
  ...
  const val private.interpolatex := function( xmin, xmax, keep, interp,
      xminbox, xmaxbox ) {
    wider gui, private, public;
    member := spaste( private.gconstructor, '.interpolatex' );
  ...
  const val public.interpolatexy := function( xmin = [], xmax = [], keep = F,
      interp = 'spline', ymin = [], ymax = [], xminbox = [], xmaxbox = [],
      yminbox = [], ymaxbox = [] ) {
    wider private;
    ...
    return( private.interpolatexy( xmin, xmax, keep, interp, ymin, ymax,
  ...
  const val private.interpolatexy := function( xmin, xmax, keep, interp, ymin,
      ymax, xminbox, xmaxbox, yminbox, ymaxbox ) {
    wider gui, private, public;
    member := spaste( private.gconstructor, '.interpolatexy' );
